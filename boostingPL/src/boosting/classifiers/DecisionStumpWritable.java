 ...
 */
public class DecisionStumpWritable 
  extends AbstractClassifier 
  implements WeightedInstancesHandler, Sourcable, Writable{
   ...
   */
	public static void main(String[] argv) {
		runClassifier(new DecisionStumpWritable(), argv);
	}
	
	
	/**
	 * ===================================================
	 * For BoostingPL on Hadoop
	 * 
	 */
	...
	public void write(DataOutput out) throws IOException {
		out.writeInt(m_AttIndex);
		out.writeDouble(m_SplitPoint);
		out.writeInt(m_numClasses);
		for (int i = 0; i < m_Distribution.length; i++) {
			for (int j = 0; j < m_Distribution[i].length; j++) {
				out.writeDouble(m_Distribution[i][j]);
			}
		}
	}
	...
	public void readFields(DataInput in) throws IOException {
		m_AttIndex = in.readInt();
		m_SplitPoint = in.readDouble();
		m_numClasses = in.readInt();
		m_Distribution = new double[3][m_numClasses];
		for (int i = 0; i < m_Distribution.length; i++) {
			for (int j = 0; j < m_Distribution[i].length; j++) {
				m_Distribution[i][j] = in.readDouble();
			}
		}
	}
}
